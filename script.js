        ...
        markerDataArray.forEach(markerData => {
            ...
            const infoWindowContent = `
                <div style="color: black; font-family: Arial, sans-serif;">
                    <h3 style="color: black; font-size: 23px; font-weight: bold;">${markerData.locationName}</h3>
                    ...
                    <div>
                        ${markerData.photos ? markerData.photos.map(photo => `
                            <img src="${photo}" alt="${markerData.locationName}" 
                                style="width: 150px; height: 150px; object-fit: cover; margin: 5px; border-radius: 10px;">
                        `).join('') : ''}
                    </div>
                    ${ratingSection}
                </div>
            ...
            marker.addListener('click', () => {
                ...
                showComments(markerData.locationName, 5); 
            });
...
function showComments(locationName, star) {
    ...
    if (currentR) {
        ...
        const selectedRating = rating[star];
        if (selectedRating && selectedRating.comments.length > 0) {
            document.getElementById("comments-title").style.display = 'block';
            document.getElementById("comments-title").innerText = `${star}-Star Comments`;

            let comments = selectedRating.comments.map(x => `
            ...
            document.getElementById("rating-comments").innerHTML = 'No comments available for this rating.';
        }
    }
}
